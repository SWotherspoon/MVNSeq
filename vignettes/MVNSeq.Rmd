...
```{r}
table(max.col(fit$P),fit.mclust$classification)
```

Match labels and compare means and variances
```{r}
j <- order(max.col(table(max.col(fit$P),fit.mclust$classification)))
sapply(fit$mvn[j],function(p) p$mu)
lapply(fit$mvn[j],function(p) p$Sigma)
```
...
```{r}
table(max.col(fit$P),s$cl)
```

Match labels and compare means 
```{r}
j <- order(max.col(table(max.col(fit$P),s$cl)))
t(s$Mu)
sapply(fit$mvn[j],function(p) p$mu)
```
and variances
