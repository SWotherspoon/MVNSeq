...
##' @return `parsMixClass` returns a list with elements
...
##' Initial parameter estimates for random effect HMM and mixture
##' models
##'
...
##' @title Initial Parameters
##' @param K the number of classes
##' @param pars lists of parameters generated by `initMix` or
##'   `initHMM`
##' @importFrom stats cov
...
##' @param y the sequence of observations
##' @param cl an integer vector allocating observations to classes
##' @param formula1 formula for the logistic model relating the
##'   transition from state 1 to state 2 to the covariates
##' @param formula2 formula for the logistic model relating the
##'   transition from state 2 to state 1 to the covariates
##' @param data a dataframe of covariates
##' @importFrom stats model.matrix glm.fit binomial coef fitted cov
...
##'
##' Perform a single step of the EM algorithm fitting a mean and
...
grmvnEMStep <- function(K,ys,pars,muv) {
  for(k in 1:K) {
    ...
    for(g in seq_along(ys)) {
      ...
      SS.err <- SS.err + crossprod(sqrt(w)*yerr)+
        wsum*((ybar-mu.a)%o%(ybar-mu.a)+var.a)
...
## Scott(2002).  Our notation differs a little from Scott's.  The
## transition matrix Q for the underlying chain is the matrix with
...
##'   transition from state 2 to state 1 to the covariates
##' @param data a dataframe of covariates
##' @importFrom stats model.matrix glm.fit binomial coef fitted
##' @importFrom mvtnorm dmvnorm
##' @return the updated set of parameters
...
##' sequences of observations. `mvnMix` fits a mixture model to a
##' single sequence of observations. `gmvnMix` and `grmvnMix` fits
##' separate mixture models to several groups (sequences) of
##' observations. `gmvnMix` fits a mixture to each group so that each
##' component has a different mean across groups, but a common
##' covariance. `grmvnMix` constrains the means of the components to
##' be Normally distributed across groups.
##'
...
##' @param common.fractions should the mixing fractions be common
##'   across groups
...
##' models to sequences of observations. `mvnHMM` fits a hidden Markov
##' model to a single sequence of observations. `gmvnHMM` and
##' `grmvnHMM` fits separate Markov models to several groups
##' (sequences) of observations. `gmvnHMM` fits a hidden Markov model
##' to each group so that each state has a different mean across
##' groups, but a common covariance. `grmvnHMM` constrains the means
##' of the states to be Normally distributed across groups.
##'
##' @title Multivariate Normal Mixture Model
##' @param y the sequence of observations
##' @param cl an integer vector allocating observations to classes
##' @param gr an integer vector allocating observations to groups
##' @param common.transition should the transition probabilities be
...
##' probabilities are governed by logistic regression. `mvnHMM2` fits
##' a hidden Markov model to a single sequence of observations.
##' `gmvnHMM2` and `grmvnHMM2` fit separate Markov models to several
##' groups (sequences) of observations. `gmvnHMM2` fits a hidden
##' Markov model to each group so that each state has a different mean
##' across groups, but a common covariance. `grmvnHMM2` constrains the
