...
initMix <- function(y,cl,K=max(cl)) {
  ...
  ## Return parameters
  list(K=K,p=tabulate(cl)/length(cl),mvn=mvn)
}
...
initHMM2 <- function(y,cl,formula1,formula2,data,K=max(cl)) {
  ...
  ## Return parameters
  list(K=K,p0=tabulate(cl)/length(cl),
...
mixEMStep <- function(K,y,pars) {
  ...
  L <- matrix(0,nrow(y),K)
  for(k in 1:K) L[,k] <- dmvnorm(y,pars$mvn[[k]]$mu,pars$mvn[[k]]$Sigma)
  ...
  list(K=K,p=p,mvn=pars$mvn,P=P,logL=logL)
}
...
hmmEMStep <- function(K,y,pars) {
  ...
  list(p=p,Q=Q,mvn=mvn,Q0=Q0,P=t(ps),logL=logL)
}
...
hmm2EMStep <- function(K,y,pars,formula1,formula2,data) {
  ...
  ## Return parameters
  list(K=K,p=p,
       beta1=beta1,q1=q1,beta2=beta2,q2=q2,
       mvn=mvn,P=t(ps),logL=logL)
}
...
gmvnMix <- function(y,cl,gr,
  ...
  list(pars=pars,logL=logL,
       AIC=-2*logL+2*nparams,BIC=-2*logL+log(nrow(y))*nparams)
}
...
grmvnMix <- function(y,cl,gr,
  ...
  nparams <- n.frac+n.grp*n.mean+n.cov
  ## Return parameters, logL, AIC, BIC
  list(pars=pars,muv=muv,logL=logL,
       AIC=-2*logL+2*nparams,AIC=-2*logL+log(nrow(y))*nparams)
}
